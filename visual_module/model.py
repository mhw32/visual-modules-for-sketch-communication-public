...
class AdaptorHigh(nn.Module):
    r"""Train an light adaptor of two linear layers
    on top of FC6 from the VGG deep network.

    Total of 1,048,833 trainable parameters.

    @param hiddens_dim: integer [default: 128]
                        number of hidden dimensions
                        in linear layer
    """
    def __init__(self, hiddens_dim=128):
        super(AdaptorHigh, self).__init__()
...
class AdaptorMid(nn.Module):
    r"""Train an adaptor with spatial attention on
    top of the CONV-4-2 layer from the VGG deep network.

    The purpose of attention is to focus on subsets
    of the rich input space with fewer parameters than
    a convolutional layer.

    Total of 1,049,115 trainable parameters.
    """
    def __init__(self):
        super(AdaptorMid, self).__init__()
        # separate attention mechanisms for photo and sketch
...
class AdaptorEarly(nn.Module):
    r"""Train an adaptor with spatial attention on top
    of the POOL-1 layer from the VGG deep network.

    Total of 1,048,839 trainable parameters.
    """
    def __init__(self):
        super(AdaptorEarly, self).__init__()
...
class Swish(nn.Module):
    r"""Swish: a self-gated activation function
